      ...
      <div className="w-full max-w-md mx-auto p-4 bg-white rounded-lg shadow">
        ...
        {menuData.map((item) => (
          <div key={item.id} className="mb-6">
            ...
            <div
              className="w-full h-40 bg-gray-200 rounded-lg overflow-hidden flex items-center justify-center cursor-pointer"
              onClick={() => item.imageUrl && openModal(item.imageUrl)}
            >
              {item.imageUrl ? (
                <Image
                  src={item.imageUrl}
                  alt={`${item.source} 메뉴 이미지`}
                  width={300}
                  height={200}
